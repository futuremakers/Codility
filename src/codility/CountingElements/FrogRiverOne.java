...
at every position across the river from 1 to X (that is, we want to find the earliest moment when all the positions
 from 1 to X are covered by leaves). You may assume that the speed of the current in the river is negligibly
...
public class FrogRiverOne {
	...
	public static int solution(int X, int[] A){
		Set<Integer> matchedPoint=new HashSet<>();
		for (int i = 0; i < A.length; i++) {
			if(A[i]>0 && A[i]<=X)
				matchedPoint.add(A[i]);
			if(matchedPoint.size()==X)
				return i;
		}
		return -1;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(solution(5,new int[]{1,5,1,4,2,3,5,4}));
		System.out.println(solution(5,new int[]{1,5,5,4,2,9,5,4,3,5}));
		System.out.println(solution(5,new int[]{}));
		System.out.println(solution(5,new int[]{1,2,3}));
